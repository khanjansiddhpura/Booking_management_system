...
<div class="container mx-auto px-4 py-8">
    <div class="bg-white rounded-lg shadow-md p-6">
        <h1 class="text-3xl font-bold text-gray-800 mb-6">
            <i class="fas fa-calendar-check"></i> Create Bookings
        </h1>

        <div class="mb-4">
            <p class="text-gray-600">Create a new booking for a customer!</p>
        </div>
        @if(session('success'))
        ...
        <form action="{{ route('bookings.store') }}" method="POST">
            ...
            <div class="mb-4" id="booking_slot_container" style="display: none;">
                ...
                <select name="booking_slot"
                    ...
                    <option value="First Half" {{ old('booking_slot') == 'First Half' ? 'selected' : '' }}>First Half (Morning)</option>
                    <option value="Second Half" {{ old('booking_slot') == 'Second Half' ? 'selected' : '' }}>Second Half (Afternoon)</option>
                </select>
                @error('booking_slot')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
